        ...
        for i in range(len(self.board)):
            ...
            for j in range(len(self.board[i])):
                ...
                pygame.draw.circle(
                    self.screen,
                    (col),
                    (cnt_j + 1 + self.cell_size // 2, cnt_i + 1 + self.cell_size // 2),
                    self.cell_size // 2 - 4,
                )
    ...
    def on_click(self, cell_coords=None):
        if cell_coords:
            if self.board[cell_coords[1]][cell_coords[0]] == 0:
                ...
                self.board[cell_coords[1]][
                    cell_coords[0]
                ] = 0  # если другой цвет то пометим синим
...
class Lines(Board):
    ...
    def road(self, mass, stx, sty, endx, endy):  # возвращает список координат пути
        ...
        while stx != endx or sty != endy:
            min_mass = [
                mass[endx - (0 < endx <= len(mass))][endy],
                mass[endx + (0 <= endx < len(mass) - 1)][endy],
                mass[endx][endy - (0 < endy <= len(mass[0]))],
                mass[endx][endy + (0 <= endy < len(mass[0]) - 1)],
            ]
